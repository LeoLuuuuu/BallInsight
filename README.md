...
索引构建层:  jieba 分词 → Whoosh 倒排索引 + sklearn TF-IDF 向量化
查询理解层:  同义词扩展 (300+ 映射) + Levenshtein 拼写纠错
排序模型层:  BM25 × 时间衰减 × 情感因子 → MMR 多样性重排
...
├── data/                  # 静态数据
│   ├── male_players.csv           # EA FC 24 球员数据
│   └── football_synonyms.json     # 中文足球同义词 (75 条)
└── docs/                  # 文档
...
```
FinalScore = BM25 × e^(-0.05 × DaysDiff) × SentimentFactor
```

- **BM25**：概率检索模型基础评分（k1=1.2, b=0.75）
- **时间衰减**：负指数衰减，半衰期约 14 天，确保新闻时效性
- **情感因子**：SnowNLP 极性分析，正向放大、负向抑制
- 支持 MMR（Maximal Marginal Relevance）多样性重排（λ=0.7）

### 2. 三模型对比评测

- **Baseline 1**：TF-IDF + 余弦相似度（向量空间模型）
- **Baseline 2**：纯 BM25（概率检索模型）
- **Ours**：BM25 × TimeDecay × Sentiment（混合模型）
- 15 条人工标注 Query，0/1/2 三级相关性打分
- 评测指标：P@10、NDCG@10
- 前端 A/B Test 面板：三栏结果对比 + 柱状图可视化
...
| 端点 | 方法 | 说明 |
|------|------|------|
| `/api/search` | GET | 主搜索 (支持 q/sentiment/offset/limit) |
| `/api/search/compare` | GET | 三模型对比搜索 (支持 mmr 参数) |
